-- BigQuery multi-table preprocessing for ICU length-of-stay prediction.
...
DECLARE top_n_input_items INT64 DEFAULT {{top_n_input_items}};
...
windowed_outputs AS (
  SELECT
    CAST(oe.ITEMID AS INT64) AS ITEMID
  FROM `{{project_id}}.{{dataset_id}}.{{outputevents_table}}` AS oe
  JOIN icu
    ON CAST(oe.ICUSTAY_ID AS INT64) = icu.ICUSTAY_ID
  WHERE oe.ICUSTAY_ID IS NOT NULL
    AND oe.ITEMID IS NOT NULL
...
CREATE OR REPLACE TABLE `{{project_id}}.{{dataset_id}}.{{selected_input_items_table}}` AS
WITH icu AS (
  SELECT
    CAST(ICUSTAY_ID AS INT64) AS ICUSTAY_ID,
    SAFE_CAST(INTIME AS TIMESTAMP) AS INTIME
...
inputs AS (
  SELECT
    CAST(ICUSTAY_ID AS INT64) AS ICUSTAY_ID,
    CAST(ITEMID AS INT64) AS ITEMID,
    SAFE_CAST(CHARTTIME AS TIMESTAMP) AS EVENT_TIME,
    SAFE_CAST(AMOUNT AS FLOAT64) AS AMOUNT
  FROM `{{project_id}}.{{dataset_id}}.{{inputevents_cv_table}}`
  WHERE ICUSTAY_ID IS NOT NULL
  ...
  SELECT
    CAST(ICUSTAY_ID AS INT64) AS ICUSTAY_ID,
    CAST(ITEMID AS INT64) AS ITEMID,
    SAFE_CAST(STARTTIME AS TIMESTAMP) AS EVENT_TIME,
    SAFE_CAST(AMOUNT AS FLOAT64) AS AMOUNT
  FROM `{{project_id}}.{{dataset_id}}.{{inputevents_mv_table}}`
  WHERE ICUSTAY_ID IS NOT NULL
),
windowed_inputs AS (
  SELECT
  ...
  WHERE inputs.ITEMID IS NOT NULL
    AND inputs.EVENT_TIME IS NOT NULL
    AND inputs.AMOUNT IS NOT NULL
    AND inputs.EVENT_TIME >= icu.INTIME
    AND inputs.EVENT_TIME <= TIMESTAMP_ADD(icu.INTIME, INTERVAL window_hours HOUR)
),
ranked_items AS (
  SELECT
    ITEMID,
    COUNT(*) AS event_count
  FROM windowed_inputs
  GROUP BY ITEMID
  ORDER BY event_count DESC
  LIMIT {{top_n_input_items}}
)
SELECT
  ranked_items.ITEMID,
  ranked_items.event_count,
...
windowed_drugs AS (
  ...
  JOIN icu
    ON CAST(pr.HADM_ID AS INT64) = icu.HADM_ID
  WHERE pr.HADM_ID IS NOT NULL
    AND COALESCE(pr.DRUG, pr.DRUG_NAME_GENERIC) IS NOT NULL
    AND SAFE_CAST(pr.STARTDATE AS TIMESTAMP) IS NOT NULL
...
SET input_exprs = (
  SELECT COALESCE(STRING_AGG(
    CONCAT(
      "MAX(IF(ITEMID = ", CAST(ITEMID AS STRING), ", total_amount, NULL)) AS input_", CAST(ITEMID AS STRING), "_sum,\n",
      "      MAX(IF(ITEMID = ", CAST(ITEMID AS STRING), ", mean_amount, NULL)) AS input_", CAST(ITEMID AS STRING), "_mean,\n",
      "      MAX(IF(ITEMID = ", CAST(ITEMID AS STRING), ", event_count, NULL)) AS input_", CAST(ITEMID AS STRING), "_count,\n",
      "      MAX(IF(ITEMID = ", CAST(ITEMID AS STRING), ", last_amount, NULL)) AS input_", CAST(ITEMID AS STRING), "_last"
    ),
    ",\n"
  ), "NULL AS no_input_features")
  FROM `{{project_id}}.{{dataset_id}}.{{selected_input_items_table}}`
);
...
output_windowed AS (
  SELECT
    CAST(oe.ICUSTAY_ID AS INT64) AS ICUSTAY_ID,
    CAST(oe.ITEMID AS INT64) AS ITEMID,
    SAFE_CAST(oe.CHARTTIME AS TIMESTAMP) AS EVENT_TIME,
    SAFE_CAST(oe.VALUE AS FLOAT64) AS VALUE
  FROM `{{project_id}}.{{dataset_id}}.{{outputevents_table}}` AS oe
  JOIN icu
    ON CAST(oe.ICUSTAY_ID AS INT64) = icu.ICUSTAY_ID
  JOIN `{{project_id}}.{{dataset_id}}.{{selected_output_items_table}}` AS selected
    ON CAST(oe.ITEMID AS INT64) = selected.ITEMID
...
input_filtered AS (
  ...
  FROM input_windowed
  JOIN icu USING (ICUSTAY_ID)
  JOIN `{{project_id}}.{{dataset_id}}.{{selected_input_items_table}}` AS selected
    ON input_windowed.ITEMID = selected.ITEMID
  WHERE input_windowed.AMOUNT IS NOT NULL
    AND input_windowed.EVENT_TIME >= icu.INTIME
    AND input_windowed.EVENT_TIME <= TIMESTAMP_ADD(icu.INTIME, INTERVAL {{window_hours}} HOUR)
),
input_aggregated AS (
  SELECT
    ICUSTAY_ID,
    ...
    COUNT(*) AS event_count,
    ARRAY_AGG(AMOUNT ORDER BY EVENT_TIME DESC LIMIT 1)[OFFSET(0)] AS last_amount
  FROM input_filtered
  GROUP BY ICUSTAY_ID, ITEMID
),
input_pivot AS (
...
LEFT JOIN input_pivot USING (ICUSTAY_ID)
LEFT JOIN drug_pivot USING (ICUSTAY_ID)
WHERE icu.LOS IS NOT NULL
""", chart_exprs, lab_exprs, output_exprs, input_exprs, drug_exprs);
...
WITH metrics AS (
  ...
  UNION ALL SELECT 'top_n_chart_items', CAST(top_n_chart_items AS FLOAT64)
  UNION ALL SELECT 'top_n_lab_items', CAST(top_n_lab_items AS FLOAT64)
  UNION ALL SELECT 'top_n_output_items', CAST(top_n_output_items AS FLOAT64)
  UNION ALL SELECT 'top_n_input_items', CAST(top_n_input_items AS FLOAT64)
  UNION ALL SELECT 'top_n_drugs', CAST(top_n_drugs AS FLOAT64)
  ...
  UNION ALL SELECT 'selected_output_items', CAST(COUNT(*) AS FLOAT64)
    FROM `{{project_id}}.{{dataset_id}}.{{selected_output_items_table}}`
  UNION ALL SELECT 'selected_input_items', CAST(COUNT(*) AS FLOAT64)
    FROM `{{project_id}}.{{dataset_id}}.{{selected_input_items_table}}`
